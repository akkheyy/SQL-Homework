...
SELECT staff.staff_id, staff.first_name, staff.last_name, SUM(payment.amount) as "sale_totals"
...
SELECT customer.customer_id, customer.first_name, customer.last_name, SUM(payment.amount) as "total_amount_paid"
...
HAVING name= "English" AND (title LIKE "K%" OR title LIKE "Q%");

-- Second Answer to 7a using queries instead of joins
SELECT title
FROM film
WHERE language_id = 1
...
SELECT film.title, COUNT(rental_id) AS "total_amount_rented"
